...
In the above, $\alpha_{1}$ is the amount of time expected to pass before a male encounters a nuptial gift. 
...
Similarly, we assume that a male will only enter the mating pool with no gift if they are unsuccessful in obtaining gift, so the probability that a male obtains no gift after $T_{m}$ is modelled as,
...
$$\gamma_{m} > \frac{\alpha_{1}M}{\sqrt{\beta}}.$$
...
Note that $\Pr(g)$ and $\Pr(n)$ need not sum to unity, and if $\Pr(L)$ is sufficiently low, then finding a male with a gift will be easier than finding a male without one (i.e., $\Pr(g) > \Pr(n)$). Also note that the expected time spent in the mating pool before a focal female encounters a male with a gift will be $1 / (\Pr(G) \times M \sqrt{\beta})$, while the expected time spent in the mating pool before a focal female encounters any male will be $1 / (M \sqrt{\beta})$ (since $\Pr(G) + \Pr(L) = 1$). We can use this to determine the rate at which a female increases her fitness when she is choosy and mates only with males that have gifts versus when she is not choosy and mates with the first male she encounters.

The rate at which a female increases her fitness by being choosy and mating only when she encounters a male with a gift is as follows,
...
We can now investigate the values of $\gamma_{m}$ and  $\gamma_{f}$ across parameter space. First, we look across values of $\alpha_{1}$ given fixed values of  $M = 1$, $\beta = 1$, $\alpha_{1} = 1/16$, and $T_{F} = 1$. Note that expected male search time for a nuptial gift will be $T_{M} = \alpha_{1}$, meaning that $exp(T_{m} / \alpha_{1}) = e$.

```{r, echo = FALSE}
gamma_m <- function(M = 1, beta = 1, alpha = 1/16, TF = 1){
  return(alpha * (M / sqrt(beta)));
}
```

```{r, echo = FALSE}
gamma_f <- function(M = 1, beta = 1, alpha = 1/16, TF = 1){
...
par(mar = c(5, 5, 1, 1));
g_m <- gamma_m(M = 1, beta = 1, alpha = alpha_vals, TF = 1);
g_f <- gamma_f(M = 1, beta = 1, alpha = alpha_vals, TF = 1);
...
alpha_vals <- seq(from = 0.0001, to = 2, by = 0.0001);
par(mar = c(5, 5, 1, 1));
g_m <- gamma_m(M = 2, beta = 1, alpha = alpha_vals, TF = 1);
g_f <- gamma_f(M = 2, beta = 1, alpha = alpha_vals, TF = 1);
plot(x = alpha_vals, y = g_m, type = "l", lwd = 2, cex.lab = 1.25,
     xlab = expression(paste("Nuptial gift search time (", alpha[1], ")")),
     ylab = expression(paste("Fitness threshold (", gamma, ")")),
     ylim = c(0, 4));
...
points(x = alpha_vals, y = g_f, type = "l", lwd = 2, cex.lab = 1.25, 
       col = "red");
text(x = 1, y = 0.4, cex = 3, labels = "A");
text(x = 0.1, y = 0.5, cex = 1, labels = "B");
text(x = 1.75, y = 2, cex = 4, labels = "C", col = "white");
text(x = 0.4, y = 3, cex = 5, labels = "D");
arrows(x0 = alpha_vals[15000], x1 = alpha_vals[13800], col = "blue",
       y0 = g_m[15000], y1 = 3.5, length = 0.1, lwd = 2);
text(x = alpha_vals[13600], y = 3.65, labels = "M", col = "blue");
arrows(x0 = alpha_vals[15000], x1 = alpha_vals[15000], col = "red",
       y0 = g_f[15000], y1 = 0.5, length = 0.1, lwd = 2);
text(x = alpha_vals[15000], y = 0.3, labels = "M", col = "red");
arrows(x0 = alpha_vals[18000], x1 = alpha_vals[18000], col = "red",
       y0 = g_f[18000], y1 = 0.5, length = 0.1, lwd = 2);
text(x = alpha_vals[18000], y = 0.3, labels = expression(T[F]), col = "red");
