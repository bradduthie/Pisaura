...
For simplicity, we assume that offspring sired by a giftless male have a fitness of $1$, so offspring sired by males providing gifts have a fitness of $1 + \gamma$. The inclusive fitness of a female whose offspring are sired by a giftless male is therefore $W_{f}(n) = \kappa (1/2)$, where $\kappa$ is total number of offspring. For simply, we assume $\kappa = 2$, meaning that female fitness is $W_{f}(n) = 1$. The inclusive fitness of a female whose offspring are sired by a male with a gift is then,

$$W_{f}(g) = 1 + \gamma\frac{\Pr(G)}{\Pr(G) + \Pr(F)}.$$
...
Given $T^{*}_{m} = `r TmOpt`$, we can now calculate $\Pr(G) = `r PrGf(Tm = TmOpt, a1 = 1/16)`$ and $\Pr(L) = `r PrLf(Tm = TmOpt, a1 = 1/16, a2 = Inf)`$. We can then feed 
...
An alternative model of male behaviour
=============================================
...
$$W_{m,G} = \frac{1 + \gamma}{\alpha_{1} + \frac{\sqrt{\beta}}{M}}.$$
