...
An alternative to all of this is to compare the fitness of a male that does versus does not go for a nuptial gift. Then calculate expected fitness as time out expected to get the nuptial gift versus no time out. Note,

$$\int_{0}^{\infty}e^{- \frac{1}{\alpha_{1}}T_{m}} = \alpha_{1}.$$

This means that $E[T_{m}] = \alpha_{1}$ for finding a nuptial gift. The fitness of a male that finds a nuptial gift is therefore,

$$W_{m,G} = \frac{1 + \gamma}{\alpha_{1} + \frac{\sqrt{\beta}}{M}}.$$

$$W_{m,F} = \frac{1}{\alpha_{2} + \frac{\sqrt{\beta}}{M}}.$$

$$W_{m,L} = \frac{1}{\frac{\sqrt{\beta}}{M}} = \frac{1}{M\sqrt{\beta}}.$$

This would suggest that finding a nuptial gift is higher fitness if,

$$\gamma > \frac{\frac{\alpha_{1}}{\sqrt{\beta}} + \frac{1}{M}}{M} - 1.$$

Or if,

$$\alpha_{1} > \left( M\left(1 + \gamma \right) - \frac{1}{M} \right)\sqrt{\beta}.$$

This is nice, and probably worth a deeper look.
